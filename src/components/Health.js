...
const Health = ({cat}) =>{
  const [Data, setData] = useState("");
    ...
    const fetchData = async() => {await axios.get(cat
      ...
      : "https://newsapi.org/v2/top-headlines?country=in&category=health&apiKey=c00c45977ede4dc7b9f22c6bc297a588"
      )
      ...
      <Row xs="1" sm="2" md="4">{ Data ? Data.map((data,index)=>(
        <Col key={index}  className='my-3'>
          <Card  className="p-3"style={{
                                width: '22rem',height:"600px",boxShadow:"2px 2px 10px silver", borderRadius:"10px"
                            }}>
            <img src={data.urlToImage} alt="image not found" className='img-fluid' style={{width:"100%",height:"200px",objectFit:"cover"}}/>
